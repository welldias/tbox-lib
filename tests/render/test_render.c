 ...
 * can load it. Render Pipeline never dereferences a box's `font` (it only
 ...
 * caller (v0 style has no "text" field, that's box->text). Individual test
 * cases override the fields they care about. */
...
/* Zero-initialized box: no parent/children/siblings, no text/font, empty
...
int tbox_test_render_run(void) {
    ...
    /* 4: non-empty text/non-NULL font -> exactly one TEXT_RUN with the
     * right text/color/face/origin. */
    {
        tbox_style style    = tbox_test_render_default_style();
        style.color         = (tbox_css_rgba){ 5, 6, 7, 255 };
        tbox_layout_box box = tbox_test_render_default_box(&style);
        box.text            = tbox_test_render_view_from_cstr("hello");
        box.font            = font;
        box.content_box     = (tbox_rect){ 15.0, 25.0, 200.0, 16.0 };

        tbox_arena arena       = tbox_arena_create(0);
        tbox_display_list list = tbox_render_build_display_list(&arena, &box);
        TBOX_TEST_ASSERT_MSG(list.count == 1, "text-only box must produce exactly one op");
        if (list.count == 1) {
            TBOX_TEST_ASSERT(list.items[0].kind == TBOX_PAINT_TEXT_RUN);
            TBOX_TEST_ASSERT_MSG(string_view_equal_cstr(list.items[0].text, "hello"), "TEXT_RUN text must match box->text");
            ...
            TBOX_TEST_ASSERT(list.items[0].rect.x == 15.0 && list.items[0].rect.y == 25.0);
        }
        tbox_arena_destroy(&arena);
    }

    /* 5: a box with both a background and text produces FILL_RECT
     * immediately before TEXT_RUN, in that order. */
    {
        tbox_style style        = tbox_test_render_default_style();
        style.background_color  = (tbox_css_rgba){ 9, 9, 9, 255 };
        tbox_layout_box box      = tbox_test_render_default_box(&style);
        box.text                 = tbox_test_render_view_from_cstr("hi");
        box.font                 = font;
    ...
    /* 6: transparent background + empty text on the root produces zero ops
     * for the root itself, but its child (with its own background) is
     * still visited and produces its own op. */
    {
        ...
        tbox_arena arena       = tbox_arena_create(0);
        tbox_display_list list = tbox_render_build_display_list(&arena, &root);
        TBOX_TEST_ASSERT_MSG(list.count == 1, "transparent root must produce no op of its own, only the child's");
        if (list.count == 1) {
